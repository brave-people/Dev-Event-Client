...
export const Create = () => {
  ...
  const createHost = async (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();

    if (!hostName) return validateForm();

    const hostImageUrl = await uploadImage();

    const body: Host = {
      host_name: hostName,
      description,
      image_link: hostImageUrl,
    };

    const data = await createHostApi({ data: body });
    if (data.status_code === STATUS_201) router.push('/admin/host');
